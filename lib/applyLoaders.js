...
module.exports = function applyLoaders(runContext, sourceCode, sourceMap, done) {
  ...
  // const runloaders = runContext.loaders.map((i)=>i.path)
  // console.log('===>', runContext)
  // console.log(' runContext.loaderContext.request',  )
  runLoaders(
    {
      resource: runContext.loaderContext.resourcePath,
      loaders:  runContext.loaders.map((i)=>(i.query ? i.path + i.query: i.path)),
      readResource: fs.readFile.bind(fs)
    }, function (error, result) {
      if(result && result.result && result.result[0]){
        done(error, result.result[0])
